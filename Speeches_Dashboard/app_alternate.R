...
ui <- fluidPage(
  titlePanel("Speeches Explorer"),
  
  # Row 1: Dynamic section (sidebar + president-specific plot)
  fluidRow(
    column(3,
           selectInput("pres_choice", "Choose a President:",
                       choices = sort(unique(df$president))),
           br(),
           uiOutput("president_img")
    ),
    column(9,
           h3("Ratings by Participant Political Leaning"),
           plotOutput("president_plot", height = "400px")
    )
  ),
  
  hr(),
  
  # Row 2 & 3 combined into a tabset
  tabsetPanel(
    tabPanel(
      "Overall Party Averages",
      h3("Overall Party Averages"),
      plotOutput("overview_plot", height = "500px")
    ),
    tabPanel(
      "Map of Presidents",
      h3("MDS of Presidents by Value Profiles"),
      plotOutput("mds_plot", height = "500px")
    )
  ),
  ...
  hr(),
  tags$div(
    style = "text-align:center; font-size: 11px; color: grey; margin-top: 10px;",
    "Code licensed under the GNU GPL v3. ",
    "Data from an academic study by Andrew Demetriou, licensed under ",
    tags$a(href="https://creativecommons.org/licenses/by/4.0/", "CC-BY 4.0"),
    ". Portraits from ",
    tags$a(href="https://commons.wikimedia.org", "Wikimedia Commons"),
    "."
  )
)
